...
class AttributesControllerWebTest extends WebTestCase
{
	...
	{
		$this->adminLogin();
		$this->open('/admin/shop/attribute/create');
		$this->type('id=ShopAttribute_title','name'.time());
		$this->type('id=ShopAttribute_name','name'.time());
		$this->select('id=ShopAttribute_type','label=Dropdown');
