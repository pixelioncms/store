...
class EEavBehavior extends CActiveRecordBehavior
{
    ...
    {
        return __CLASS__ . $this->tableName . $this->attributesPrefix . $this->getOwner()->tableName() . $this->getModelId(); // . $this->getModelId()
    }
    ...
    public function loadEavAttributes($attributes)
    {
        ...
        $this->cache->set($this->getCacheKey(), $this->attributes->toArray(), 3600 * 12);
        // Return model.
        return $this->getOwner();
    }
    ...
    public function withEavAttributes2($attributes = array())
    {
        // If not set attributes, search models with anything attributes exists.
        if (empty($attributes)) {
            $attributes = $this->getSafeAttributesArray();
        ...
        // $attributes be array of elements: $attribute => $values
        $criteria = $this->getFindByEavAttributesCriteria2($attributes);
        // Merge model criteria.
        $this->getOwner()->getDbCriteria()->mergeWith($criteria);
        // Return model.
        return $this->getOwner();
