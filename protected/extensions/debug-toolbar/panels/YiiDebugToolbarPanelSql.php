        ...
        foreach ($logs as $log) {
            ...
            if (0 === strncasecmp($message, 'begin:', 6)) {
                ...
                if (null !== ($last = array_pop($stack)) && $last[0] === $token) {
                    ...
                    throw new CException(YiiDebug::t('Mismatching code block "{token}". Make sure the calls to Yii::beginProfile() and Yii::endProfile() be properly nested.',
                        array('{token}' => $token)
                    ));
            }
        }
