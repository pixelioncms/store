...
<?php
Yii::app()->clientScript->registerScript('compare-index', "
...
<div class="col-sm-5 compare-page">
    ...
    <div class="scroller2 mCustomScrollbar">
        ...
        <ul class="list-unstyled compare-categories-list text-uppercase">
            ...
            foreach ($this->model->products as $id => $group) {
                ...
                ?>
                <li class="<?= $class ?>"><?= Html::link($group['name'], array('/compare', 'cat_id' => $id)) ?></li>
            <?php } ?>
        </ul>
    ...
    <table class="table">
        ...
        if (isset($this->model->attributes[$cat_id]['filter_name'])) {
            foreach ($this->model->attributes[$cat_id]['filter_name'] as $attribute) {
                if ($isType) {
                    $unq = array();
                ...
                if ($flag) {//$flag
                    ...
                    ?>
                    <tr>
                        <td class="attr"><b data-toggle="tooltip" data-placement="top"
                                            title="<?= $attribute->title ?>"><?= CMS::truncate($attribute->title, 30) ?></b>
                        </td>
    ...
    <?php if (count($gp) > 0) { ?>
        <div class="scroller mCustomScrollbar">
            <ul class="compare-products-list list-unstyled">
                <?php foreach ($gp[$cat_id]['items'] as $index => $p) { ?>
                    <?php
