...
/**
 * Main markups model
 * This is the model class for table "shop_markup".
 *
 * The followings are the available columns in table 'shop_markup':
 * @property integer $id
 ...
 */
class ShopMarkup extends ActiveRecord {

    const MODULE_ID = 'markup';

    /**
     * @var array ids of categories to apply markup
     */
    protected $_categories;

    /**
     * @var array ids of manufacturers to apply markup
     */
    ...
    public function getGridColumns() {
        return array(
            array(
                'name' => 'name',
                'type' => 'raw',
                'value' => 'Html::link(Html::encode($data->name), array("/admin/shop/markup/update", "id"=>$data->id))',
            ),
            array(
                'name' => 'sum',
                'value' => '$data->sum'
    ...
    public function getForm() {
        ...
        return array(
            ...
            'elements' => array(
                'content' => array(
                    'type' => 'form',
                    'title' => Yii::t('MarkupModule.default', 'Общая информация'),
                    'elements' => array(
                        'name' => array(
                            'type' => 'text',
                        ),
    ...
    public function getCategories() {
        ...
        $this->_categories = Yii::app()->db->createCommand()
                ->select('category_id')
                ->from('{{shop_markup_category}}')
                ->where('markup_id=:id', array(':id' => $this->id))
                ->queryColumn();
    ...
    public function afterSave() {
        ...
        if (!empty($this->_manufacturers)) {
            foreach ($this->_manufacturers as $id) {
                Yii::app()->db->createCommand()->insert('{{shop_markup_manufacturer}}', array(
                    'markup_id' => $this->id,
                    'manufacturer_id' => $id,
                ));
            }
        ...
        if (!empty($this->_categories)) {
            foreach ($this->_categories as $id) {
                Yii::app()->db->createCommand()->insert('{{shop_markup_category}}', array(
                    'markup_id' => $this->id,
                    'category_id' => $id,
                ));
            }
    ...
    /**
     * Clear markup manuacturer and category
     */
    public function clearRelations() {
        Yii::app()->db->createCommand()->delete('{{shop_markup_manufacturer}}', 'markup_id=:id', array(':id' => $this->id));
        Yii::app()->db->createCommand()->delete('{{shop_markup_category}}', 'markup_id=:id', array(':id' => $this->id));
    }
