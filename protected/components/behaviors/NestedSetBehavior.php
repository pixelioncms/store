...
class NestedSetBehavior extends CActiveRecordBehavior {
    ...
    public function getLeftValue() {
        return $this->Owner->attributes[$this->leftAttribute];
    }

    protected function getLevelValue() {
        return $this->Owner->attributes[$this->levelAttribute];
    }
    ...
    public function getRightValue() {
        return $this->Owner->attributes[$this->rightAttribute];
    }

    public function getTree2($returnrootnode = true) {
        Yii::trace(get_class($this) . '.getTree()', 'components.behaviors.NestedSetBehavior');

        $condition = $this->leftAttribute . " >= ? AND " . $this->rightAttribute . " <= ?";
        $params = array($this->getLeftValue(), $this->getRightValue());
        $builder = $this->Owner->getCommandBuilder();
        $criteria = $builder->createCriteria($condition, $params);
        $criteria->order = $this->leftAttribute . " asc";
     ...
     * @return boolean whether the saving succeeds.
     */
    public function saveNode($mSuccess = true, $mError = true, $runValidation = true, $attributes = null) {
        return $this->save($mSuccess = true, $mError = true, $runValidation, $attributes);
    }
