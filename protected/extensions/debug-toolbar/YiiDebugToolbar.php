...
Yii::import('debug-toolbar.panels.*');
Yii::import('debug-toolbar.widgets.*');
...
class YiiDebugToolbar extends CWidget
{
     ...
     * The base URL that contains all published asset files of debug-toolbar.
     * @return string
     */
        ...
        foreach ($this->getPanels() as $id => $config) {
            if (!is_object($config)) {
                ...
                if (false === ($panel instanceof YiiDebugToolbarPanelInterface)) {
                    throw new CException(YiiDebug::t('The %class% class must be compatible with YiiDebugToolbarPanelInterface', array(
