...
class XUploadAction extends CAction {
    ...
    public function init() {
        ...
        if ($this->subfolderVar === null) { //Yii::app()->request->getQuery($this->subfolderVar, date("mdY"))
            $this->_subfolder = $this->subfolderVar;
        } else if ($this->subfolderVar !== false) {
            $this->_subfolder = Yii::app()->request->getQuery($this->subfolderVar);
        }
    ...
    protected function handleDeleting() {
        if (isset($_GET["_method"]) && $_GET["_method"] == "delete") {
            ...
            if ($_GET["file"]) {
                ...
                $file = ProjectsImages::model()->findByAttributes(array('filename' => $_GET["file"]));
                if ($file)
                    unlink(Yii::getPathOfAlias("webroot.uploads.{$file->project_id}") . DS . $file->filename);
                $isCover = $file->cover;
                $pid = $file->project_id;

                $file->delete();
                if ($isCover) {
                    $find = ProjectsImages::model()->findByAttributes(array(
                        'project_id' => $pid,
                            ));
                    if ($find) {
                        $find->cover = 1;
                        $find->save(false, false);
                    }
                }

                // $this->deleteFile($this->getFileUrl().$model->filename);
    ...
    protected function handleUploading() {
        ...
        if ($model->{$this->fileAttribute} !== null) {
           ...
           // $model->{$this->displayNameAttribute} = $model->{$this->fileAttribute}->getName();
            $model->{$this->displayNameAttribute} = CMS::gen(10) . '.' . $model->{$this->fileAttribute}->getExtensionName();
            ...
            if ($model->validate()) {
                ...
                if ($returnValue === true) {
                    ...
                    $file = new ProjectsImages;
                    $file->filename = $model->{$this->displayNameAttribute};
                    $file->project_id = $this->_subfolder;
                    if ($file->save(false, false)) {
                        $findAll = ProjectsImages::model()->findAllByAttributes(array(
                            'project_id' => $file->project_id,
                            'cover' => 1
                                ));
                        if (!$findAll) {
                            $file->cover = 1;
                            $file->save(false, false);
                        }
                        $path = $this->getPath();
                    ...
                    $json['files'][] = array(
                        ...
                        "thumbnailUrl" => $model->getImageUrl('filename',$file->project_id,'100x100'), //$model->getThumbnailUrl($this->getPublicPath())
            ...
            if ($returnValue === true) {
                ...
                $json = array();
                $files = ProjectsImages::model()->findAllByAttributes(array('project_id' => $this->_subfolder));

                $i = 0;
                $numItems = count($files);
                foreach ($files as $file) {
                    $i++;
                    //  $test = CUploadedFile::getInstanceByName($file->filename);
                    /*
                      if(++$i === $numItems) {
                      $fileCover = ProjectsImages::model()->findByAttributes(array(
                      'project_id' => $file->project_id,
                      'filename'=>$file->filename
                      ));
                      $fileCover->cover=1;
                      $fileCover->save(false,false,false);
                      } */
                    $json['files'][] = array(
                        "name" => $file->filename,
                        "cover" => $file->cover,
                        "project_id" => $file->project_id,
                        "type" => mime_content_type(Yii::getPathOfAlias('webroot') . $this->getFileUrl($file->filename)),
                        "size" => filesize(Yii::getPathOfAlias('webroot') . $this->getFileUrl($file->filename)),
                        "thumbnailUrl" => $file->getImageUrl('filename', $file->project_id, '100x100'), //$this->getFileUrl($file->filename),
                        "deleteUrl" => $this->getController()->createUrl($this->getId(), array(
                            "_method" => "delete",
                            "file" => $file->filename,
                        )),
