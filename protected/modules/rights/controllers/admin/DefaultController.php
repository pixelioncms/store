...
class DefaultController extends AdminController
{
        ...
        if ($assignSelectOptions !== array()) {
            ...
            if (isset($_POST['AssignmentForm']) === true) {
                ...
                if ($formModel->validate() === true) {
                    ...
                    Yii::app()->user->setFlash($this->module->flashSuccessKey, Rights::t('default', 'Permission :name assigned.', array(':name' => $item->getNameText()))
                    );

                    $this->redirect(array('admin/default/user', 'id' => $model->getId()));
                }
            }
        } // No items available
        else {
    ...
    public function actionRevoke()
    {
        ...
        if (Yii::app()->request->isPostRequest === true) {
            ...
            Yii::app()->user->setFlash($this->module->flashSuccessKey, Rights::t('default', 'Permission :name revoked.', array(':name' => $item->getNameText()))
            );

            // if AJAX request, we should not redirect the browser
            if (isset($_POST['ajax']) === false)
                $this->redirect(array('admin/default/user', 'id' => $_GET['id']));
        } else {
