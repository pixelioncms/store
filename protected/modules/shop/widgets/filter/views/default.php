...
<script>
    ...
    $(function () {
        ...
        $("#filter-form").on("change", function (event) { //submit
            ...
            $.fn.yiiListView.update('shop-products', {data: form,type:$(this).attr('method')});
            ...
            if (serializeJSON !== undefined) {
                //delete serializeJSON.token;
                var test = [serializeJSON];




               /* $.each(test.filter, function (name, val) {
                    var label;
                    $.each(val, function (i, id) {
                        label = $('#' + name + '_' + id).next('label');
