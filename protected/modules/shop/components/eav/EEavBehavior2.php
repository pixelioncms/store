...
class EEavBehavior extends CActiveRecordBehavior
{
    ...
    protected function getFindByEavAttributesCriteria($attributes)
    {
        ...
        /* $test= ShopAttribute::model()->findByAttributes(array('name'=>'slider'));
...
sort($values);
    //@todo foreach() @ fixed bug with dublicated IN (..)
                //foreach ($values as $value) {
                ...
                /*if(strpos($value,'-')){
                    ...
                    $criteria->join .= "\nJOIN {{shop_attribute_option}} opt$i"
                        . "\nON eavb$i.{$this->valueField} = opt$i.id";
                        //. "\nON opt$i.{$id} = eavb$i.{$this->valueField}";



                    $criteria->addCondition("opt1$i.value1 >= ".$min);
                    $criteria->addCondition("opt$i.value1 <= ".$max);
                    //Yii::log($attribute,'info','application');
                    //$criteria->addCondition("eavb$i.{$this->valueField} >= " . $min);
                    //$criteria->addCondition("eavb$i.{$this->valueField} <= " . $max);
                    //$criteria->addInCondition("opt1$i.{$this->valueField}1", range($min,$max));
                }else{*/
...
if(count($values)>1){
    ...
    //where
                ...
                //$criteria->addBetweenCondition("eavb$i.{$this->valueField}",10,20);
                // }
                $i++;
                // }
