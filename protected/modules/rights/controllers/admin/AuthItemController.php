...
class AuthItemController extends AdminController {
    ...
    public function actionPermissions() {
        $this->pageName = Rights::t('default', 'Permissions');
        $this->breadcrumbs = array(
            Rights::t('default', 'MODULE_NAME') => Rights::getBaseUrl(),
            $this->pageName,
        );
        //$this->topButtons = CHtml::link(Rights::t('default', 'Generate items for controller actions'), array('authItem/generate'), array('class' => 'btn btn-success'));
        $this->topButtons = array(array(
                'label' => Rights::t('default', 'Generate items for controller actions'),
                'url' => array('/admin/rights/authItem/generate'),
       ...
       /* if (Yii::app()->hasModule(strtolower($moduleName))) {
        ...
        $columns = array(
            array(
                'name' => 'description',
                'header' => Rights::t('default', 'Item'),
                'type' => 'html',
                // 'value'=>'".$data->description."',
                //  'value' => '"<span class=\"badge badge-success\">".$data->description."</span>"',
                'htmlOptions' => array(
    ...
    public function actionOperations() {
        $this->pageName = Rights::t('default', 'Operations');
        $this->breadcrumbs = array(
            Rights::t('default', 'MODULE_NAME') => Rights::getBaseUrl(),
            $this->pageName,
        );
        ...
        // $this->topButtons = CHtml::link(Rights::t('default', 'Create a new operation'), array('authItem/create', 'type' => CAuthItem::TYPE_OPERATION), array('class' => 'btn btn-success'));
        $this->topButtons = array(array(
                'label' => Rights::t('default', 'Create a new operation'),
                'url' => array('/admin/rights/authItem/create', 'type' => CAuthItem::TYPE_OPERATION),
                'htmlOptions' => array('class' => 'btn btn-success')
        ));
        Yii::app()->user->rightsReturnUrl = array('/admin/rights/authItem/operations');
        ...
        $dataProvider = new RAuthItemDataProvider('operations', array(
            'type' => CAuthItem::TYPE_OPERATION,
                ...
                //     'url' => $this->createUrl('/admin/rights/authItem/sortable'),
                //  ),
        ));
    ...
    public function actionTasks() {
        $this->pageName = Rights::t('default', 'Tasks');
        $this->breadcrumbs = array(
            Rights::t('default', 'MODULE_NAME') => Rights::getBaseUrl(),
            $this->pageName,
        );
        ...
        Yii::app()->user->rightsReturnUrl = array('/admin/rights/authItem/tasks');
        // $this->topButtons = CHtml::link(Rights::t('default', 'Create a new task'), array('authItem/create', 'type' => CAuthItem::TYPE_TASK), array('class' => 'btn btn-success'));
        $this->topButtons = array(array(
                'label' => Rights::t('default', 'Create a new task'),
                'url' => array('/admin/rights/authItem/create', 'type' => CAuthItem::TYPE_TASK),
                'htmlOptions' => array('class' => 'btn btn-success')
        ));
    ...
    public function actionRoles() {
        Yii::app()->user->rightsReturnUrl = array('/admin/rights/authItem/roles');
        $this->pageName = Rights::t('default', 'Roles');
        $this->breadcrumbs = array(
            Rights::t('default', 'MODULE_NAME') => Rights::getBaseUrl(),
            $this->pageName,
        );
        ...
        // $this->topButtons = CHtml::link(Rights::t('default', 'Create a new role'), array('authItem/create', 'type' => CAuthItem::TYPE_ROLE), array('class' => 'btn btn-success'));
        $this->topButtons = array(array(
                'label' => Rights::t('default', 'Create a new role'),
                'url' => array('/admin/rights/authItem/create', 'type' => CAuthItem::TYPE_ROLE),
                'htmlOptions' => array('class' => 'btn btn-success')
        ));
        ...
        if (isset($_POST['GenerateForm']) === true) {
            ...
            if ($model->validate() === true) {
                ...
                if (($generatedItems = $generator->run()) !== false && $generatedItems !== array()) {
                    Yii::app()->getUser()->setFlash($this->module->flashSuccessKey, Rights::t('default', 'Authorization items created.')
                    );
                    $this->redirect(array('/admin/rights/authItem/permissions'));
                }
            }
        }
    ...
    public function actionCreate() {
        ...
        if (isset($_POST['AuthItemForm']) === true) {
            ...
            if ($formModel->validate() === true) {
                ...
                Yii::app()->user->setFlash('success', Rights::t('default', ':name created.', array(':name' => $item->getNameText()))
                );

                // Redirect to the correct destination
                $this->redirect(Yii::app()->user->getRightsReturnUrl(array('/admin/rights/authItem/permissions')));
            }
        }
    ...
    public function actionUpdate() {
        ...
        if (isset($_POST['AuthItemForm']) === true) {
            ...
            if ($formModel->validate() === true) {
                ...
                Yii::app()->user->setFlash('success', Rights::t('default', ':name updated.', array(':name' => $item->getNameText()))
                );
        ...
        if ($childSelectOptions !== array()) {
            ...
            if (isset($_POST['AuthChildForm']) === true) {
                ...
                if ($childFormModel->validate() === true) {
                    ...
                    if (is_array($childFormModel->itemname)) {
                        foreach ($childFormModel->itemname as $itemname) {
                            $this->_authorizer->authManager->addItemChild($itemName, $itemname);
                            $child = $this->_authorizer->authManager->getAuthItem($itemname);
                            $child = $this->_authorizer->attachAuthItemBehavior($child);
                            $names[] = Html::tag('span', array('class' => 'label label-default'), $child->getNameText(), true);
                        }
                    }


                    $fullName = implode(' ', $names);
                    // Set a flash message for adding the child //$this->module->flashSuccessKey
                    Yii::app()->user->setFlash('success', Rights::t('default', 'Child :name added.', array(':name' => $fullName)));

                    // Reidrect to the same page
                    $this->redirect(array('/admin/rights/authItem/update', 'name' => urlencode($itemName)));
                }
            }
        } else {
            $childFormModel = null;
        ...
        $this->pageName = Rights::t('default', 'Update :name', array(
                    ':name' => $model->name,
    ...
    public function actionDelete() {
        ...
        if (Yii::app()->request->isPostRequest === true) {
            ...
            Yii::app()->user->setFlash('success', Rights::t('default', ':name deleted.', array(':name' => $item->getNameText())));
            ...
            throw new CHttpException(400, Rights::t('default', 'Invalid request. Please do not repeat this request again.'));
        }
    }
    ...
    public function actionRemoveChild() {
        ...
        if (Yii::app()->request->isPostRequest === true) {
            ...
            Yii::app()->user->setFlash($this->module->flashSuccessKey, Rights::t('default', 'Child :name removed.', array(':name' => $child->getNameText()))
            );
    ...
    public function getType() {
        ...
        if (in_array($type, $validTypes) === true)
            ...
            throw new CException(Rights::t('default', 'Invalid authorization item type.'));
    }
