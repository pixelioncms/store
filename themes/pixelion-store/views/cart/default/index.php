...
<div class="table-responsive">
    <table class="table table-striped">
        ...
        <?php foreach ($items as $index => $product) { ?>
            <?php
            // print_r($product);die;
            $price = ShopProduct::calculatePrices($product['model'], $product['variant_models'], $product['configurable_id'],$product['quantity']);
            ?>
            ...
            <tr id="product-<?= $index ?>">
                ...
                <td>
                    ...
                    echo Html::link(Html::encode($product['model']->name), array('/shop/product/view', 'seo_alias' => $product['model']->seo_alias)) . '<br/>';
                    ...
                    if (!empty($product['variant_models'])) {
                        ...
                        foreach ($product['variant_models'] as $variant) {
                            //print_r($variant);
                            $add_price = ($variant->price > 0) ? ' (+' . $variant->price . ' ' . Yii::app()->currency->active->symbol . ')' : '';
                            echo ' - ' . $variant->attribute->title . ': ' . $variant->option->value . $add_price . '<br/>';
                        }
                ...
                <td class="cart-product-sub-total">
                                <span class="cart-sub-total-price">
                                    ...
                                    echo Html::openTag('span', array('class' => 'price','id'=>'price-unit-'.$index));
...
$units = $product['model']->getUnits();

                                    ?>
                                    <br/>/ <?= $units[$product['model']->unit]; ?>
                                </span>
                </td>
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="cart-grand-total">
                Всего к оплате:
                <span id="total" class="inner-left-md"><?= Yii::app()->currency->number_format($totalPrice) ?></span>
