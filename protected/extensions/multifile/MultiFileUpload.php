...
class MultiFileUpload extends CMultiFileUpload {
    ...
    public function run() {
       ...
       // else
            echo Html::fileField($name, '', $this->htmlOptions);
    ...
    public function registerClientScript() {
        ...
        if (is_dir($assets)) {
            $cs->registerScriptFile($baseUrl . '/jquery.MultiFile.js',CClientScript::POS_END);
            $cs->registerCssFile($baseUrl . '/multifile.css');
        }else
            throw new Exception(Yii::t('app',"multifile - Error: Couldn\'t find assets folder to publish."));

        $cs->registerScript('Yii.MultiFileUpload#' . $id, "jQuery('#{$id}').MultiFile({$options});",CClientScript::POS_END);
    }
