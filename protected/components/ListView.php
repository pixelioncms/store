 ...
 *
 * <pre>
 * $dataProvider=new ActiveDataProvider('Post');
 *
...
class ListView extends CListView {
    ...
    public $pager = array('class' => 'LinkPager'); //LinkPager
    /**
    ...
    public $htmlLoading = '<div class=\"grid-loading\"></div>';
    ...
    /**
     * @var mixed the URL for the AJAX requests should be sent to. {@link Html::normalizeUrl()} will be
    ...
    public function init() {
       ...
       // if ($this->baseScriptUrl === null)
        ...
        $this->pager = CMap::mergeArray(array(
                    'cssFile' => $pagerFile,
                    'htmlOptions' => array('class' => 'pagination ' . $htmlClass),
                    'header' => (Yii::app()->controller instanceof AdminController) ? '' : (isset($this->pager['header']))?$this->pager['header']:false
    ...
    public function registerClientScript() {
        ...
        if ($this->ajaxUrl !== null)
            $options['url'] = Html::normalizeUrl($this->ajaxUrl);
        ...
        $cs->registerScriptFile($assets . '/jquery.yiilistview.js', CClientScript::POS_END);

        $cs->registerScript(__CLASS__ . '#' . $id, "jQuery('#$id').yiiListView($options);", CClientScript::POS_END);
        //if(Yii::app()->request->isAjaxRequest) $cs->reset();
    }
    ...
    public function renderItems() {
        echo Html::openTag($this->itemsTagName, array('class' => $this->itemsCssClass)) . "\n";
        ...
        if (($n = count($data)) > 0) {
            ...
            foreach ($data as $i => $item) {
                ...
                if (isset($data['data']['access'])) {
                    $render = Yii::app()->access->check($data['data']['access']);
                } else {
                    $render = true;
                }
        ...
        echo Html::closeTag($this->itemsTagName);
    }
