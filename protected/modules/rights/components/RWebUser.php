...
class RWebUser extends CWebUser {
    ...
    public function openAccess($operation) {
        if (is_array($operation)) {
            foreach ($operation as $op) {
                $res = $this->checkAccess($op);
                if ($res)
                    break;
            }
            return $res;
        }else {
            return $this->checkAccess($operation);
        }
    }
    ...
    public function checkAccessRole($operation, $params = array(), $allowCaching = true) {
        // Allow superusers access implicitly and do CWebUser::checkAccess for others.
    ...
    public function getRightsReturnUrl($defaultUrl = null) {
        ...
        return $returnUrl !== null ? Html::normalizeUrl($returnUrl) : Html::normalizeUrl($defaultUrl);
    }
