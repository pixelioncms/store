...
class Rights {
    ...
    public static function getAuthItemOptions() {
        return array(
            CAuthItem::TYPE_OPERATION => Rights::t('default', 'Operation'),
            CAuthItem::TYPE_TASK => Rights::t('default', 'Task'),
            CAuthItem::TYPE_ROLE => Rights::t('default', 'Role'),
        );
    }
    ...
    public static function getAuthItemTypeNamePlural($type) {
        switch ((int) $type) {
            case CAuthItem::TYPE_OPERATION: return Rights::t('default', 'Operations');
            case CAuthItem::TYPE_TASK: return Rights::t('default', 'Tasks');
            case CAuthItem::TYPE_ROLE: return Rights::t('default', 'Roles');
            default: throw new CException(Rights::t('default', 'Invalid authorization item type.'));
        }
    }
    ...
    public static function getAuthItemRoute($type) {
        switch ((int) $type) {
            case CAuthItem::TYPE_OPERATION: return array('/admin/rights/authItem/operations');
            case CAuthItem::TYPE_TASK: return array('/admin/rights/authItem/tasks');
            case CAuthItem::TYPE_ROLE: return array('/admin/rights/authItem/roles');
            default: throw new CException(Rights::t('default', 'Invalid authorization item type.'));
        }
    }
    ...
    public static function getRolesOld($column = 'name') {
        $roles = Yii::app()->db->createCommand()
            ->select($column)
            ->from('{{authitem}}')
            ->where('type=2')
        ...
        if ($type !== (int) $type) {
            foreach ($items as $itemName => $item) {
                //if ($item->type == 1) {
                 $moduleName = '';
            if(strpos($itemName, '.')){
                    $arrayItems = explode('.', $itemName);
            
                    list($mod, $controller, $action) = $arrayItems;
               ...
               //     $moduleName = '';
                //}
                $selectOptions[$moduleName . self::getAuthItemTypeNamePlural($item->type)][$itemName] = $item->getNameText();
            }
        }
