...
Yii::import("app.phpoffice.phpword.src.PhpWord.Element.Section");
Yii::import("app.phpoffice.phpword.src.PhpWord.Media");
Yii::import("app.phpoffice.phpword.src.PhpWord.PhpWord");
Yii::import("app.phpoffice.phpword.src.PhpWord.Shared.ZipArchive");
Yii::import("app.phpoffice.phpword.src.PhpWord.Writer.AbstractWriter");
Yii::import("app.phpoffice.phpword.src.PhpWord.Writer.WriterInterface");
/**
 * Word2007 writer
 */
class Word2007 extends AbstractWriter implements WriterInterface
{
    ...
    public function __construct(PhpWord $phpWord = null)
    {
        ...
        $this->parts = array(
            ...
            'Part_Settings'       => 'word/settings.xml',
        ...
        foreach (array_keys($this->parts) as $partName) {
            $named = get_class($this);
            $partClass = $partName;
            Yii::import("app.phpoffice.phpword.src.PhpWord.Writer.{$named}.Part.{$partName}");
            //$partClass = get_class($this) . '\\Part\\' . $partName;

            if (class_exists($partClass)) {
                /** @var \PhpOffice\PhpWord\Writer\Word2007\Part\AbstractPart $part Type hint */
                $part = new $partClass();
                if(in_array($partClass,array('Part_Settings','WebSettings','Part_Footnotes','Part_Endnotes'))){
                    //$oReflectionClass = new ReflectionClass($partClass); 
                  //  $s  = new Part_Settings();
                    //print_r($oReflectionClass->getMethods());die;
                    
                   // echo "app.phpoffice.phpword.src.PhpWord.Writer.{$named}.Part.{$partName}";
                  //  print_r($part);die;
                //}else{
               // print_r($this);
                  //  $s->setParentWriter($this);
