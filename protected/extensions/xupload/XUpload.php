...
class XUpload extends CJuiInputWidget {
    ...
    public function run() {
        ...
        Yii::app()->clientScript->registerScript(__CLASS__ . '#' . $this->htmlOptions['id'], "jQuery('#{$this->formID}').fileupload({$options});", CClientScript::POS_READY);
        $htmlOptions = array();
        if ($this->multiple) {
    ...
    public function publishAssets() {
        ...
        if (is_dir($assets)) {
            ...
            $cs->registerScriptFile('//blueimp.github.io/JavaScript-Templates/js/tmpl.min.js', CClientScript::POS_END);
            $cs->registerScriptFile('//blueimp.github.io/JavaScript-Load-Image/js/load-image.all.min.js', CClientScript::POS_END);
            $cs->registerScriptFile('//blueimp.github.io/JavaScript-Canvas-to-Blob/js/canvas-to-blob.min.js', CClientScript::POS_END);

            $cs->registerScriptFile('//blueimp.github.io/Gallery/js/jquery.blueimp-gallery.min.js', CClientScript::POS_END);
            ...
            if ($this->useCover)
                $cs->registerScriptFile($baseUrl . '/js/cover.js', CClientScript::POS_HEAD);
  ...
  'use strict';
    ...
    $('#" . $this->formID . "').fileupload();
        ...
        // Load existing files:
        $('#" . $this->formID . "').addClass('fileupload-processing');
           // console.log($('#" . $this->formID . "').fileupload('action','url'));
        $.ajax({
            // Uncomment the following to send cross-domain cookies:
            //xhrFields: {withCredentials: true},
            url: '" . $this->url . "',
            dataType: 'json',
             context: $('#" . $this->formID . "')[0]
        }).always(function () {
...
", CClientScript::POS_END);
            $cs->registerScript('XuploadI18N', $js, CClientScript::POS_END);
            /**
